...
.qdoc.parser.tags[enlist"@ignore"]:`.qdoc.parser.tag.ignore;
.qdoc.parser.tags[enlist"@dictkey"]:`.qdoc.parser.tag.dictkey;
.qdoc.parser.tags[enlist"@public"]:`.qdoc.parser.tag.public;
...
.qdoc.parser.parse:{[fileName]
    ...
    / read in the file and map to line numbers
    file:update linenum:til count line from ([]line:read0 fileName);
    file@:where not in [;" \t}"] first each file`line;

    / Remove block comments
    file:file where null{$[x=`;$[y;`C;z;`E;x];x=`C;$[z;`;x];x]}\[`] . file[`line] like/:1#/:"/\\";   
    funcSignatures:file[`line] where not"/"=first each file`line;
    ...
    / mark each line which is a function
    file:update isfunc:1b from file where line in funcSignatures;
    / want to use 3.6 features.  If the next linenumber is a comment after a function, then look forward for the next group of comments
    / use these instead of the preceeding block
    file:update lookforward:1b from file where next[linenum]=1+linenum,isfunc,not next isfunc;
    / mark contiguous blocks of comments (no gaps)
    file:update contblock:sums (1<deltas linenum)or isfunc from file;
     
    / create a grouping of blocknumber!table row numbers
    / drop any blocks which are function definitions
    g:group file`contblock;
    
    / get the row numbers for forward looking
    file:update commentrows:1 _' g[contblock] from file where lookforward;   
    / and looking back
    g:(exec contblock from file where isfunc,not lookforward) _ g;
    file:update commentrows:g[contblock-1] from file where isfunc,not lookforward; 
    
    commentsDict:key[funcAndArgs]!trim over file[`line]exec commentrows from file where isfunc;
    ...
    tagDiscovery:{key[.qdoc.parser.tags]!{(w!(w:where sum x) cut til count first x)@/:where each x}like[x;]@/:"*",/:key[.qdoc.parser.tags],\:"*" } each commentsDict;
    tagComments:commentsDict@'tagDiscovery;
    comments:commentsDict@'(til each count each commentsDict) except' raze each raze each tagDiscovery;
...
.qdoc.parser.parseTags:{[func;tagsAndComments]
    ...
    parsed:{
        ...
        / the <br> puts the line break back into the comment
        :tagFunc[z;"<br>" sv/: y x];
...
.qdoc.parser.postProcess:{[funcAndArgs;comments;tagComments]
    ...
    / Remove functions which have the @ignore tag set to true
    ignored:key[tagComments]where {0<count first value[x]first where key[x]~\:"@ignore"} each value tagComments;
    
    :distinct (,/)(assignmentInFunc;dictKeysNoComments;nonDeclaredFuncs;ignored);
 };
...
/ @ignore
.qdoc.parser.tag.public:{[func;pub]
    /Returns 1b if tag=@public else 0b
    :("S"$pub)~enlist `$"@public"
 };

/ @ignore
.qdoc.parser.tag.dictkey:{[func;dictkeys]
    dDict:flip `name`keyname`types`description!"SS**"$\:();

    if[()~dictkeys;
        :dDict;
    ];

    paramSplit:1_/:" " vs/:dictkeys;
    / the name of the dictionary parameter this relates to
    dictNames:"S"$paramSplit@\:0;
    paramNames:"S"$paramSplit@\:1;
    paramDescs:" " sv/:3_/:paramSplit;

    paramTypes:paramSplit@\:2;
    paramTypes:.qdoc.parser.typeParser[func;] each paramTypes;
  
    :dDict upsert flip (dictNames;paramNames;paramTypes;paramDescs);
 };
...
/ if ignore tag is present, then ignore the function (don't care about value of tag)
.qdoc.parser.tag.ignore:{[func;ignore]
  :not ()~ignore;
 };
